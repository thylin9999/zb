...
import ApplyHostInfo from './ApplyInfo/Host.vue'
...
export default {
    ...
    data () {
        return {
            ...
            applyInfo: {},
            appliedInfo: {}, // 申请信息
            isConfirmed: false, // 是否已经通过实名验证
            isApplied: false// 是否已经申请
        }
    },
    computed: {
        comName () {
            return this.steps.find(x => x.id === this.currentStep).comName
        }
    },
    created () {
        this.init()
    ...
    methods: {
        ...
        changeStatus (step) {
            ...
            // 第二步提交后，设置第三步的状态为 3=审核中，2=失败， 1=成功
            this.appliedInfo.status = step
        },
        applyAgain () {
            // this.currentTab = 2
